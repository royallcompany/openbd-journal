;( function( $, window, document, undefined ) {
    ...
    Plugin.prototype = {
        ...
        attachToggle: function( $tree ) {
            ...
            if ( this.options.toggleAllButton ) { // TODO
                $tree.before( '<button type="button" class="at-toggle-all ' + this.options.toggleAllClass + '">Expand all</button>' );
            }
        ...
        attachToggleClick: function( $tree ) {
            ...
            $tree.siblings( TOGGLE_CLASS_SELECTOR + '-all' ).on( CLICK_EVENT, function( event ) {
                var allToggler = $( this ),
                    $listItem = $tree.find( LIST_ITEM_SELECTOR );
                // beside tree

                if ( allToggler.text() === 'Expand all' ) {
                    self.expand( $listItem, event );
                    allToggler.text( 'Collapse all' );
                } else {
                    self.collapse( $listItem, event );
                    allToggler.text( 'Expand all' );
                }
            } );
        },
